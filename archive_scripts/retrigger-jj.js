require('dotenv').config({path: '.env'}); // Load the real key
...
async function main() {
  ...
  console.log(`Found ${failedJJ.length} failed JJ invoices. Retriggering Gemini AI directly...`);
  ...
  for (const inv of failedJJ) {
    ...
    if (!fs.existsSync(actualPath)) {
        console.log("File missing!"); continue;
    }
    ...
    if (geminiRes.ok) {
       ...
       try {
           ...
           if (ocr.totalAmount) {
              ...
              console.log("=> FAIL: No totalAmount in JSON");
           }
       } catch (e) {
