...
async function run() {
  ...
  const sales = await prisma.sale.aggregate({ _sum: { grossSales: true, netPaid: true }, where: { store: 'Tasty Bun', is2025: false, weekEnd: { gte, lte } } });
  const wages = await prisma.staffWage.findMany({ where: { is2025: false, weekEnd: { gte, lte } } });
  
  let wageSum = 0;
  for(const w of wages) {
    if (w.store === 'Tasty Bun') wageSum += w.amount;
  ...
  let expSum = 0;
  for(const e of exps) {
    if(e.store === 'Tasty Bun') expSum += e.amount;
  ...
  for(const i of invs) {
    ...
    if(fr === 'Tasty Bun') invSum += i.amount;
    else if (fr === 'Combined' || !fr) invSum += i.amount * 0.5;
  }
  
  console.log('Tasty Bun Sales July:', sales._sum, 'Wages:', wageSum, 'Expenses:', expSum, 'Invoices:', invSum);
  console.log('Tasty Bun Net Profit July:', sales._sum.netPaid - wageSum - expSum - invSum);
}
